...
int min_missao(mundo *m){ // auxiliar para  encontrar minimo de tentativass
...
int max_missao(mundo *m){   //aux para max de tentativas
    int max = -1;
    for (int i = 0; i < m->Nmissoes; i++){
...
void avisa(struct evento *avisa, mundo *m) {
    ...
    // ✅ RETIRA IDs DA FILA
    while (cjto_card(avisa->b->presentes) < avisa->b->lotacao && 
           fila_tamanho(avisa->b->espera) > 0) {
...
void missao(struct evento *mev, mundo *m) {
    ...
    mev->mi->tentativas++;
    m->eventos++;
    printf("%d: MISSAO %d TENT %d HAB REQ: [ ", mev->tempo, mev->mi->id, mev->mi->tentativas);
    ...
    if (m->NCompostosV > 0 && (mev->tempo % 2500 == 0) && base_mais_proxima != NULL) {
        ...
        if (heroi_exp != NULL) {
            ...
            struct evento *ev = cria_evento(mev->tempo, heroi_exp, base_mais_proxima, 8, mev->mi);
            if (ev) fprio_insere(m->LEF, ev, ev->tipo, ev->tempo);
            return;
        }
    }
...
void fim(struct evento *fim, mundo *m) {
    ...
    for (int i = 0; i < m->Nherois; i++) {
        ...
        if (h->vivo) {
            printf("HEROI %2d VIVO PAC %3d VEL %4d EXP %4d HABS [ ", 
                   h->id, h->paciencia, h->velocidade, h->experiencia);
        } else {
    ...
    if (m->Nmissoes > 0) 
        porcentagem = ((float)m->missoes_completas / (float)m->Nmissoes) * 100.0f;
    ...
    printf("TENTATIVA / MISSAO: MIN %d MAX %d MEDIA %.0f\n", 
           minimo_missao, maximo_missao, media);

    float mortalidade = ((float)m->mortos / (float)m->Nherois) * 100.0f;
